...
const UserAdmin = ({ users, success, reload, userTitle }) => {
    ...
    const verifyUsers = () => {
        if (users.data.length == 0) {
            return <tr><td colSpan="7"><div className="flex justify-left bg-red-200 text-red-700 text-md p-2"><HiLightBulb className="text-2xl text-yellow-600" /> Não há usuários a serem mostradas no momento. Clique no botão criar usuário para adicionar.</div></td></tr>;
        }
    };
    ...
    const searchData = (e) => {
        ...
        Inertia.post(route('usuario.search'), { search });
    };
        ...
        if (reload == true) {
            ...
            return (
                <InertiaLink
                    as="button"
                    type="button"
                    href={route('usuario.create')} className="flex items-center mb-2 md:mb-0 bg-blue-500 hover:bg-blue-600 px-5 py-2 text-sm shadow-sm border-2 border-white text-gray-100 rounded-lg hover:shadow-lg">
                    <HiPlus className="text-xl" /> Usuário
                </InertiaLink>
            )
        }
    };
            ...
            <Layout>
                ...
                <div className="rounded py-2 px-4 text-gray-900 bg-gray-100 shadow">
                    ...
                    <div className="overflow-x-auto px-0 py-4 flex justify-center">
                            ...
                            <tbody>
                                {users.data.map((user, index) => (
                                    <tr key={index} className="border-b hover:bg-orange-100 bg-gray-100">
                                        ...
                                        <td className="p-2">{user.rule == 1 ? "Administrador" : "Usuário" }</td>
                                        ...
                                        <td className="p-2 flex justify-end">
                                            <InertiaLink
                                                ...
                                                as="button"
                                                type="button"
                                                href={route('usuario.edit', user.id_user)}
                                            ...
                                            <button
                                                onClick={(e) => { setIsModalVisible(true); setIdUser(user.id_user) }}
                        ...
                        {isModalVisible ?
                            <ModalUser onClose={() => setIsModalVisible(false)} onIdUser={idUser}>
                                {/* <!--body--> */}
                                <div className="text-center p-5 flex-auto justify-center">
                                    ...
                                    <p className="text-sm text-gray-500 px-8">
                                        Tem certeza de que deseja deletar este usuário? Este processo não pode ser desfeito!
                                    </p>
                                </div>
